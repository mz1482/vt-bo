...
def acq_max(ac, gp, y_max, bounds, n_warmup=10000, n_iter=250):
    """
...
class my_utility_function(object):
...
#         self.xi = xi
    def __init__(self, kind, kappa, xi, kappa_decay=1, kappa_decay_delay=0):
        ...
        if kind not in ['ucb', 'ei', 'poi','entropy_exp_gp','var_exp_gp','mode_exp_gp','cv_exp_gp','ucb_exp_gp']:
            err = "The utility function " \
                  "{} has not been implemented, " \
                  "please choose one of ucb, ei, poi, entropy_exp_gp, var_exp_gp, mode_exp_gp, cv_exp_gp.".format(kind)
    ...
    def utility(self, x, gp, y_max):
        ...
        if self.kind == 'entropy_exp_gp':
            return self._entropy_exp_gp(x, gp, y_max, self.xi)
        if self.kind == 'var_exp_gp':
            return self._var_exp_gp(x, gp, y_max, self.xi)
        if self.kind == 'mode_exp_gp':
            return self._mode_exp_gp(x, gp, y_max, self.xi)
        if self.kind == 'cv_exp_gp':
            return self._cv_exp_gp(x, gp, y_max, self.xi)
        if self.kind == 'ucb_exp_gp':
            return self._ucb_exp_gp(x, gp, y_max, self.xi)
        if self.kind == 'test':
            return self._test(x, gp, self.kappa)
    ...
    @staticmethod
    def _entropy_exp_gp(x, gp, y_max, xi):
        mean, std = gp.predict(x, return_std=True)
        en = np.log(std*np.exp(mean+0.5)*np.sqrt(2*3.1416))
        return en
    
    @staticmethod
    def _var_exp_gp(x, gp, y_max, xi):
        mean, std = gp.predict(x, return_std=True)
        ev = np.exp(2*mean+std**2)*(np.exp(std**2)-1)
        
    @staticmethod
    def _cv_exp_gp(x,gp,y_max,xi):
        mean, std = gp.predict(x, return_std=True)
    ...
    @staticmethod
    def _mode_exp_gp(x,gp,y_max,xi):
        mean, std = gp.predict(x, return_std=True)
        return np.exp(mean - std**2)
        
    @staticmethod
    def _ucb_exp_gp(x, gp, y_max,kappa):
        mean, std = gp.predict(x, return_std=True)
        mean_log_normal = np.exp(mean + std**2/2)
        var_log_normal = np.exp(2*mean+std**2)*(np.exp(std**2)-1)
        return mean_log_normal + kappa * np.sqrt(var_log_normal)
    
    @staticmethod
    def _test(x, gp, kappa):
        mean, std = gp.predict(x, return_std=True)
        return mean + kappa * std
